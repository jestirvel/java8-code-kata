...
public class Exercise8Test extends ClassicOnlineStore {
	...
	public void itemsNotOnSale() {
		 ...
		 * sale in any shop.
		 */
		List<String> itemListOnSale = shopStream.flatMap(s -> s.getItemList().stream()).map(Item::getName).distinct()
				.collect(Collectors.toList());
		Set<String> itemSetNotOnSale = customerStream.flatMap(c -> c.getWantToBuy().stream()).map(Item::getName)
				.filter(n -> itemListOnSale.stream().noneMatch(i -> n.equals(i))).collect(Collectors.toSet());
	...
	public void havingEnoughMoney() {
		 ...
		 */
		List<Item> onSale = shopStream.flatMap(s -> s.getItemList().stream()).distinct().collect(Collectors.toList());
		ToIntFunction<Item> mapper = item -> onSale.stream().filter(s -> s.getName().equals(item.getName())).sorted((a, b) -> a.getPrice() - b.getPrice()).findFirst().map(Item::getPrice).orElse(0);
		Predicate<Customer> havingEnoughMoney = c -> c.getBudget() >= c.getWantToBuy().stream().mapToInt(mapper).sum();
